...
public partial class WebHelper : IWebHelper
{
    ...
    public virtual string GetStoreLocation(bool? useSsl = null)
    {
        ...
        //prefer the configured store URL when available so generated absolute links
        //stay stable across reverse proxies, Docker host aliases, and install-time requests
        try
        {
            var configuredStoreUrl = _storeContext.Value.GetCurrentStore()?.Url;
            if (!string.IsNullOrWhiteSpace(configuredStoreUrl) && Uri.TryCreate(configuredStoreUrl, UriKind.Absolute, out _))
                storeLocation = configuredStoreUrl;
        }
        catch
        {
            //database may not be configured yet during installation - fall through to use request host
        }
        ...
        {
            //if no configured store URL is present, fall back to the current request host
        ...
        if (string.IsNullOrEmpty(storeLocation))
        {
            ...
            {
                //database not available during installation, return a default
