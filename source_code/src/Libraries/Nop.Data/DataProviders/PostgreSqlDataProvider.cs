...
public partial class PostgreSqlDataProvider : BaseDataProvider, INopDataProvider
{
    ...
    /// <summary>
    /// Pre-assigns identity values to new entities (Id == 0) by reading the table's
    /// sequence directly. We inline the sequence name in the SQL text and cast it to
    /// regclass, so the value is never bound as a string parameter. This avoids the
    /// global citext parameter coercion in LinqToDBPostgreSQLDataProvider.SetParameter
    /// which would otherwise produce an invalid "nextval(citext)" call.
    /// </summary>
    /// <param name="dataConnection">Open data connection</param>
    /// <param name="entities">Entities about to be bulk inserted</param>
    /// <typeparam name="TEntity">Entity type</typeparam>
    protected virtual void PreassignIdentities<TEntity>(DataConnection dataConnection, IList<TEntity> entities) where TEntity : BaseEntity
    {
        var newEntities = entities.Where(e => e.Id == 0).ToList();
        ...
        try
        {
            sequenceName = GetSequenceName<TEntity>(dataConnection);
        }
        catch
        {
            //entity has no identity column - nothing to pre-assign, let bulk copy use the supplied keys
            return;
        }
        ...
        var ids = dataConnection
            .Query<int>($"SELECT nextval('{sequenceName}'::regclass) FROM generate_series(1, {newEntities.Count});")
            .ToList();

        for (var i = 0; i < newEntities.Count && i < ids.Count; i++)
            newEntities[i].Id = ids[i];
    }
    ...
    public override async Task BulkInsertEntitiesAsync<TEntity>(IEnumerable<TEntity> entities)
    {
        ...
        PreassignIdentities(dataContext, entityList);
        await dataContext.BulkCopyAsync(new BulkCopyOptions() { KeepIdentity = true }, entityList);
    }
    ...
    public override void BulkInsertEntities<TEntity>(IEnumerable<TEntity> entities)
    {
        ...
        var entityList = entities.ToList();
        PreassignIdentities(dataContext, entityList);
        dataContext.BulkCopy(new BulkCopyOptions() { KeepIdentity = true }, entityList);
    }
